...
{
	// uri, artist, album, song title, song length
	return 5;
}
...
QVariant playlist_qt_model::data(QModelIndex const &index, int role) const
{
	...
	if (entry != 0)
	{
		switch (index.column())
		{
			case 0: return QString(entry->uri_.get_full().c_str());
			...
			case 3: return QString(get_metadata_value < std::string > (entry->metadata, "title", "").c_str());
			case 4:
			{
...
{
/*	boost::optional < uint64_t > index = playlist_.get_entry_index(uri_);
	if (!index)
		return;*/
