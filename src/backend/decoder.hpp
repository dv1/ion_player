	...
	*/
	std::string get_songinfo_str(bool const fill_autogenerated_data = true) const
	{
		...
		if (fill_autogenerated_data)
		{
			...
			value["num_ticks"] = static_cast < unsigned int > (get_num_ticks());
			value["num_ticks_per_second"] = static_cast < unsigned int > (get_num_ticks_per_second());

			{
				Json::Value title = value.get("title", "");
				if (title.asString().empty())
					value["title"] = get_uri().get_basename();
			}
		}
