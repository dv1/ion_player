#ifndef ION_BACKEND_BACKEND_DECODER_HPP
#define ION_BACKEND_BACKEND_DECODER_HPP
...
public:
	...
	*/
	...
	/**
	* Gets the song info. It uses JSON to store song information.
	* @return JSON value containing the song info
	*/
	virtual Json::Value get_songinfo() const = 0;
	...
	* @return String representation of the JSON value containing the song info
	*/
	std::string get_songinfo_str(bool const fill_autogenerated_data = true) const
	{
		Json::Value value = get_songinfo();

		if (fill_autogenerated_data)
		{
			value["decoder_type"] = get_type();
			value["num_ticks"] = boost::lexical_cast < std::string > (get_num_ticks());
			value["num_ticks_per_second"] = boost::lexical_cast < std::string > (get_num_ticks_per_second());

			{
				Json::Value title = value.get("title", "");
				if (title.asString().empty())
					value["title"] = get_uri().get_path();
			}
		}
